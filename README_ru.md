...
DeerFlow поддерживает приватную базу знаний через самоуправляемые векторные БД ([Qdrant](https://qdrant.tech/), [Milvus](https://milvus.io/)). Эмбеддинги общие для загрузки и поиска; настройте `RAG_EMBEDDING_*` в `.env` (см. [руководство по настройке](docs/configuration_guide.md)). Примеры в `.env.example`.
...
- 📥 **Пайплайн загрузки документов**
  - Асинхронный пайплайн загрузки документов (PDF, DOCX и т.д.) с парсером MinerU и чанкером Chonkie. Пример конфигурации в `conf.yaml` в секции `INGESTION_PIPELINE`; см. [руководство по настройке](docs/configuration_guide.md) и [документацию API](docs/API.md).
...
- 🔍 **Поиск и извлечение**
  ...
  - Поддержка приватной базы знаний

- 📃 **Интеграция RAG**
  - Самоуправляемые векторные БД: [Qdrant](https://qdrant.tech/), [Milvus](https://milvus.io/)
  - Указание файлов из RAG-провайдеров в поле ввода
